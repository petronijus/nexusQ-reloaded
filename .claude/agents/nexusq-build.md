...
| `cc: fatal error: cannot execute 'cc1': posix_spawnp` | ⚠️ **TWO causes — check both, in this order.** (1) A **concurrent** pmbootstrap zapping the buildroot mid-compile; the volume is single-writer (2026-08-31). (2) **crossdirect loses `argv[0]`** on its qemu hand-off for any compile-AND-link invocation, so GCC resolves its prefix to the crossdirect wrapper dir and hunts `cc1` there (2026-09-16). Cause 2 is **already fixed** by `pmaports-patches/0001-crossdirect-absolute-argv0.patch`, applied at pmaports checkout | `docker ps` first; let any other build finish, never restart into it. Then confirm the patch landed — the log prints `crossdirect: argv[0] patch applied, pkgrel -> 2` and Phase 7bc must build `crossdirect-5.3.1-r2`; `docker-build.sh` hard-fails if the patch does not apply (re-cut it against the pinned tree). 🚨 **Do NOT reach for `!pmb:crossdirect`, `--no-cross` or `NEXUSQ_NO_CROSS=1` to get past this.** `!pmb:crossdirect` maps to `CrossCompile.QEMU_ONLY` — pure emulation, not cross-native — and an agent that took that route on 2026-09-16 was reverted: crossdirect demonstrably built these same aports through device r98 on 09-12, so a failure here is a regression to find, not a mode to abandon |
