...
> U-Boot headroom flagged under v1.15.0 is now **92 KB** (v1.17.0, 6 721 536 B). On the Prague Q by
...
> blocked NM autoconnect and the `down` branch did nothing by design. **r93** adds
...
>
> ### Step 7 — the latency creep ✅ CLOSED 2026-08-26 — there is no creep
> Written from `module-loopback` logging `Source minimum latency increased to
> 16 → 26 → 36 ms` and read as a slow runaway needing a multi-day capture. The
> journal says otherwise: those three lines arrive **within one second of each
> other**, every time the loopback loads, and always stop at 36 ms —
> `11:09:03 → 16.00`, `11:09:04 → 26.00`, `11:09:04 → 36.00`. It is the module
> discovering the source's minimum latency at start-up and settling, not growth
> over hours. The 11 occurrences since 2026-08-25 are one ramp per reload, and
...
> | CPU busy over the window | ~2 % | **31.42 %** |
>
...
> never suspends. **Structurally identical to what Step 6 removed from USB
> audio.** The watcher behaves correctly throughout — it suspends `usb_in` — but
> cannot help: Roon holds the sink awake, and an awake sink keeps the USB
...
> `module-alsa-source` + `module-loopback` (see its lines ~85-89) and never
...
>   is resolved with it: `down_threshold=60` is no longer live-only and fragile,
>   it ships in the package (`nexusq-cpufreq-tune`) and reads 60 on the device
>   after today's two reboots.
> - **kernel r49 + nexusq-control r31** → kernel is **6.12.12-r52**, control is
>   **r33**, both installed and published.
> - **app 1.14.0+35** → released, and long superseded: **1.16.2+46** is on the
>   phone and in the OTA manifest.
...
| HDMI audio | ✅ works, selectable | _(Updated 2026-09-20, device r104 / nexusq-control r48)_ the hardware path was fine all along — a tone on `hw:HDMI,0` is audible and `HDMI_WP_AUDIO_CTRL` goes `0x20`→`0xc05e0020`. What was missing was userspace: PA had no sink (the card is `PULSE_IGNORE`'d, so `_list_outputs` dropped the row that had existed since 4a1a871 and had never once been shown), and the HDMI **video** output must be running for audio to exist at all. `nq-hdmi hold` keeps it lit, `nq-cec` announces us. Cannot wake a sink that has dropped HPD — the PHY needs a LINK_CONNECT interrupt, measured. See docs/2026-09-20-hdmi-audio-the-output-that-was-never-offered.md |
