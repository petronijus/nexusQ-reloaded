...
| 🔄 **OTA self-update** (device daemons + app) | ✅ | **the Q updates its own software over the air — no reflash, no adb** (post-v1.11.0 dev). A **signed apk repo on GitHub Pages** (`petronijus.github.io/nexusQ-reloaded/nexusq`, gh-pages) hosts the daemons (`nexusq-control` · `nexusqd` · `nexusq-btagent` · `nexusq-setupd`); the device already trusts the `pmos@local` build key baked in `/etc/apk/keys`, so `apk` installs our signed packages straight from it. The app's **Nexus Q** Settings section runs `checkNexusUpdate`/`installNexusUpdate` (`nexusq-control` **r20**). **LED-narrated** (nexusqd **r11**): the dedicated **mute LED blinks amber** = "update available" (the ring stays on your theme), a **determinate ring progress bar** while installing, a **green** flash on success. **Proven end-to-end live 2026-08-02** — took the reference Q `nexusqd` r10 / control r16 → r11 / r19 from the app, no cable. Scope = the small daemons only (`device-google-steelhead` is ~191 MB, over GitHub's 100 MB limit → waits on a glibc-rt split; the kernel stays a fastboot flash). The **companion app also self-updates** on its own track (1.9.5). See `docs/2026-08-02-device-ota-and-wifi-nogw-heal.md` + PROTOCOL §12 |
