...
| 🚄 **CPU freq scaling** 350 → **1200 MHz** | ✅ | DVFS · v1.4.0 · governor **`conservative`** since **v1.8.2** — a measured 2026-07-13 idle study showed `ondemand` kept 74 % of idle at ≥700 MHz on microburst wakeups (~1000/s); `conservative` won the A/B/C test and idle now **settles at 350 MHz** (was 56.7 % residency 2026-07-13; **60.5 %** on the first clean 14 h overnight MQTT-window measurement 2026-08-13 — and the same-day attribution found the top idle consumer was our own `nq-healthd` at ~6.3 % of a core, fixed the same day by the **r71 fork diet**: −60 % healthd CPU, idle fork rate 14→3.2/s — and then the #2 consumer, our own `nexusqd` at ~4.4 % / 22 wakeups per s, by the **r13 idle diet**: **0.165 % of a core, 2.9 wakeups/s**; ~**12 pp of one core** of constant idle background removed in one day — total idle busy **18.2 % → ~7.7 %** of one core, fork rate **13.96 → 2.59/s**, with `nq-healthd` back at **#1 (2.43 %)** and `brcmf` WiFi wakeups (~40/s) now the dominant wakeup source; see `docs/2026-08-13-idle-opp-residency-measurement.md`). History: `conservative` v1.5.0–v1.6.5 → `ondemand` v1.6.6–v1.8.1 → `conservative` v1.8.2 (this time measurement-backed) |
...
| 🎚 **USB Audio input** — the Q as a USB DAC | ✅ | **the Q takes audio IN over USB, no solder / no Bluetooth** (shipped in **v1.12.0**, device **r70** — first bring-up r65 · kernel **r46** · `nexusq-control` r16 · app 1.7.0+16). The Q has **no optical/HDMI/line input** — all its ports are OUTPUTS (verified: DTS `mcasp0` is DIT/TX → `spdif-dit`, and the OMAP4 HDMI is DSS output-only via TPD12S015A; TI docs + teardown), so USB is the only no-solder digital input. Plug a computer/phone into the micro-USB → the Q enumerates as a **USB speaker** ("Nexus Q"); kernel `CONFIG_USB_CONFIGFS_F_UAC2` (module `usb_f_uac2`) + a `uac2.0` function on the composite gadget (`c_chmask=3`/`c_srate=48000`/`c_ssize=2`/`p_chmask=0` — a *speaker*, not a mic) surface the host audio as the `UAC2Gadget` ALSA capture card, and `nexusq-uac2-in` bridges it into PulseAudio via a **stable-clock snd-aloop hop** (`alsaloop --sync=simple → hw:Loopback → PA usb_in → default sink` — the proven Roon pattern; **v1.12.0**, device r69/r70). A **4th per-service app switch** ("USB Audio", default-OFF). A TV's optical/HDMI can't feed it (that needs a receiver-chip mod); a computer/phone with USB-audio out can. ✅ **The multi-minute playback delay + idle CPU/heat are FIXED (2026-08-09, device r65; committed `2dccd3a` + pushed, OTA published — gh-pages `d983b3f`)** — the old PA bridge's `module-alsa-source` reported a bogus uptime-growing latency that poisoned `module-loopback`'s resampler (backlog grew to minutes) and its never-corked sink-input kept the amp/DAC/DMA running 24/7 (~15–20 % CPU + heat in silence). `alsaloop` rate-matches from the **real hardware pointers** with **bounded** buffers so the delay can't run away; measured live: lip-sync correct (Petr-confirmed), ~0 % CPU (was 15–20 %), die **93 °C → 76–79 °C**. `--sync=simple` (not `samplerate`: the device's `alsa-utils` has no libsamplerate). **Since v1.12.0 (device r69/r70) the r65 exclusivity trade-off is GONE** — the alsaloop feeds a stable-clock snd-aloop card and PA reads *that* instead of the async gadget endpoint (which is what caused the original runaway), so USB audio is a **mixing PA source** again: mixes with Spotify/AirPlay/Roon, unified PA volume (`nq-vol` back on `@DEFAULT_SINK@`), and the LED visualizer pulses to USB playback. (r70 fixed the live-test silence: `module-stream-restore` restored the loopback sink-input muted — now forced unmuted/unity. Live-verified: plays, lip-sync holds ~130 ms, mixing works.) *(History: r65–r69 the bridge was EXCLUSIVE — direct alsaloop→TAS5713, PA sink suspended, hw-mixer volume.)* See `docs/2026-08-02-usb-audio-input.md` + `docs/2026-08-08-usb-audio-playback-delay-and-ota-publish.md` |
...
| 🔴 **LED music visualizer** | ✅ | the ring dances to the beat · v1.6.2 · **5 selectable visualisations** + breathing color themes · idle-keepalive (no more dark-after-idle AVR starvation) · v1.6.5 · **volume-independent** — re-tapped to the active output's PA monitor + an AGC (auto-gain) so it reacts to the music at any listening volume, no low-volume flicker · v1.6.15 · **tap now gated on playback** so the amp sink suspends when idle (idle CPU ~7 % → ~1 %) · v1.7.1 · **the gate is event-driven and the ring renders adaptively** — a persistent `pactl subscribe` replaces the old 1.5 s `pactl` poll (timed re-count demoted to a 30/60 s safety net) and the render loop drops from 20 fps to 1 Hz once the frame is bit-identical *and* the ring is intent-idle (screensaver locked/blanked, no overlay/fade/spin), snapping back instantly on a key, a control command or a new stream: nexusqd idle **4.4 % → 0.165 % of a core, 22 → 2.9 wakeups/s** · nexusqd r13, 2026-08-13 |
| 📱 **Companion app** + LAN control bridge | ✅ | Flutter remote → `nexusq-control` (TCP 45015, mDNS): volume · breathing LED theme + brightness · **visualisation picker** · now-playing · v1.6.3 · reachable over WiFi · v1.6.5 · **output selector** (Holo-dark segmented control) + volume/mute now act on the active PA sink · v1.6.15 · **NFC tap-to-send receiver** (HCE — the Q taps a message onto the phone, shown as a SnackBar) + **auto-reconnect on resume/drop** (no more app-kill after backgrounding) · v1.7.0 · **two-way volume sync** — the app slider now tracks the **physical dome dial** and the LXQt applet (bridge `pactl subscribe` → `volumeChanged`) · v1.7.3 (verified live, not yet in a flashed image) · **Devices screen** (app **1.2.0+7**, device **v1.10.0**): *Pair a phone* / *Add a mouse or keyboard* / paired list with *Forget* / **HDMI desktop toggle**, from the home app bar. ⚠️ **no design review yet** — tested functionally only · **Debug mode** (Devices → Developer, app **1.3.1+9**): an always-on in-app connection log (method names only, never params) — it found the v1.10.1 btagent fd-leak on the first try. The app is versioned on its **own independent track**, deliberately NOT aligned to image releases · **runs on iOS since 2026-08-03** (verified on the iPhone 17 simulator, iOS 26.5): discovery via **native Bonjour** (NWBrowser — iOS forbids raw-socket mDNS without a restricted entitlement); first-time BT setup + self-update stay **Android-only** (no public iOS RFCOMM API; apk hand-off) — once the Q is on WiFi, iOS controls it fully. See `docs/2026-08-03-ios-companion-port.md` · **Health panel** (app **1.12.0+31**, 2026-08-10 — released as `app-v1.12.0`, OTA offer live): Settings → *Device health* — live MQTT-fed vitals/OPP-residency/service/WiFi view with a "Connect to MQTT" dialog (creds in the platform secure store) · **1.12.1+32** fixed the panel's grey-screen crash (null cast on absent problem-flag fields) · **1.13.0+33** — the dialog's Save **also provisions the Q's broker credentials** over the control link (`setMqttConfig`, PROTOCOL **§13**, `nexusq-control` **r28**) — the app is the device's only credential input, nothing hand-edited over ssh (1.12.1/1.13.0 on the phone via adb, OTA release imminent) |
| 🔄 **OTA self-update** (full system + app) | ✅ | **the Q updates its own software over the air — no reflash, no adb** (shipped in **v1.12.0**). A **signed apk repo on GitHub Pages** (`petronijus.github.io/nexusQ-reloaded/nexusq`, gh-pages) hosts the packages; the device already trusts the `pmos@local` build key baked in `/etc/apk/keys`, so `apk` installs our signed packages straight from it. **Two tracks, two Settings items:** **App update** (the phone app + the four device daemons `nexusq-control`/`nexusqd`/`nexusq-btagent`/`nexusq-setupd`, versioned together as the companion system — `checkNexusUpdate`/`installNexusUpdate`, `nexusq-control` **r25**), and **System** — the whole-appliance *apt upgrade* (`checkSystemUpdate`/`installSystemUpdate`: `apk upgrade --available` for **every** package, base musl/systemd/python + our config + daemons, **minus the kernel** — proven live upgrading systemd 261.1→261.2, reboots when base libc/init churns). **The glibc-rt split** (new aport `nexusq-glibc-rt`, `device-google-steelhead` r62) dropped the device-config apk from ~191 MB to 58 KB so the config now ships over OTA too (the ~182 MB glibc-rt base + the kernel stay flash-only; adopting the split needs **one** reflash). **LED-narrated** (nexusqd **r11**): mute-LED **amber** blink = "app update available" (ring stays on your theme); the daemon install shows a **determinate ring progress bar**, the system install an **indeterminate spinner** (slow/unknown length), a **green** flash on success. **Proven end-to-end live 2026-08-02** — daemons r10/r16→r11/r19 from the app, no cable. Companion app **1.11.0** (own track). ✅ **Fixed (2026-08-08, Option A): the System track used to report "system update failed" even though the packages installed** — the `postmarketos-mkinitfs`/`boot-deploy` trigger failed (`No kernel found in /boot`, empty plain dir on this ramdisk-less device) and left a persistent pending trigger. Fixed by restoring the kernel payload (vmlinuz + dtbs) to `/boot` so boot-deploy succeeds (it never flashes a partition — `deviceinfo_flash_kernel_on_update` is unset); live device cleaned + `docker-build.sh` now ships a populated `/boot`. ✅ **Kernel OTA (Phase-2) SHIPPED 2026-08-18** — `nexusq-kernel-ota` applies a kernel without a cable via a trial slot (p8) with a health-gated promote; **A/B rootfs** (p13/p14 + rescue initramfs, proven both directions) followed 2026-08-20, and the kernel/rootfs-ab apks are published to the OTA repo since 2026-08-23 (see `docs/2026-08-18-kernel-ota-phase2.md` + `docs/2026-08-20-rescue-initramfs-and-ramdisk-address.md`; kernel updates stay a deliberate attended action — the stock u-boot has no fallback). See `docs/2026-08-08-system-ota-mkinitfs-trigger-failure.md`. See `docs/2026-08-02-full-system-ota-and-glibc-rt-split.md` + `docs/2026-08-02-device-ota-and-wifi-nogw-heal.md` + PROTOCOL §12 |
| 📊 **MQTT health telemetry** → Home Assistant + app | ✅ | **shipped 2026-08-10, in the v1.12.0 release (commit `b49b536`, device-OTA gh-pages `cff585f`, app `app-v1.12.0` live)** — a new pure-Python-stdlib daemon (**`nexusq-mqtt`** 0.1.0-r0, noarch; MQTT 3.1.1 subset: CONNECT+auth+LWT, QoS0+retain, PINGREQ dead-link detection, reconnect+backoff) publishes the Q's health every 30 s: retained `nexusq/health/state` JSON + `nexusq/status` online/offline LWT + retained **Home Assistant MQTT discovery** (12 sensors + 6 binary_sensors — die temp, CPU freq/governor, per-OPP residency shares, load, memory, WiFi RSSI, volume, uptime; Spotify/AirPlay/Roon/USB-Audio running, LED-daemon + health-sampler problem flags). Data = `nq-healthd`'s `health.jsonl` tail (only when fresh ≤60 s) + the daemon's own sampling (`time_in_state` OPP deltas, `iw` RSSI, the mixer that currently owns the output, cgroup service states). **Deployed live 2026-08-10: 18 entities in HA with real values.** Broker creds (`/etc/nexusq/mqtt.json`, 0600) are a per-home secret — **never baked into the public image**; an unprovisioned device skips the service cleanly. **The companion app is the device's only credential provisioner** (follow-up later 2026-08-10, Petr's direction: `setMqttConfig`/`getMqttStatus`, PROTOCOL **§13**, `nexusq-control` **r28**; the Q logs in with the household broker user — a dedicated `nexusq` broker user was created, rejected and deleted the same day). The app (**1.12.0**, provisioning since **1.13.0**) reads the same feed: Settings → **"Device health"** panel with a "Connect to MQTT" dialog (creds in the platform secure store; Save also provisions the Q). Device r67 (`depends += nexusq-mqtt`). Live-proven end-to-end (app → provision → device publishing → HA + phone panel). ✅ **Fixed 2026-08-13 (`nexusq-mqtt` **r2**, OTA-published + live): the LED alarm is now a device-side VERDICT.** Every idle Q used to report **"LED ring frame is stalled" permanently**, ~10 min after the music stopped — the app thresholded the raw `led_stall` counter, which measures the ring's frame *content* staying identical, exactly what the screensaver does **by design** (locks at 300 s, blanks at 600 s, while the 1 Hz AVR keepalive re-commits the same bytes). The payload now carries **`led_stalled`** = `led_stall ≥ 6` **AND** nexusqd distressed (`nq_resp`/`nq_progress`) — the same co-signal `nq-healthd` uses to pick crit `led_frozen` over info `led_static`, so daemon and telemetry agree by construction; `led_stall` stays as a diagnostic number, and a **19th entity** appears in HA (`binary_sensor` **"LED ring"**, problem/diagnostic — an *absent* field reads healthy, never an invented alarm). Live: `binary_sensor.nexus_q_led_ring = off` with `led_stall=17, led_stalled=False`. The companion-app side (`healthProblems()` reads `led_stalled`) is **written + unit-tested but NOT released yet** — it needs an APK build + Petr's approval. See `docs/2026-08-10-mqtt-health-telemetry.md` (§7 = the provisioning follow-up) + `docs/2026-08-13-led-stall-verdict-and-progress-window.md` |
...
1.11.0 ─ ✦ step 3 · streaming services: AirPlay (shairport-sync) · first-boot rootfs resize · Roon Bridge (glibc/bwrap, validated vs a real Core) · Settings screen + per-service toggles/logs · fastboot over ssh (patch 0044)   2026-07-31   (released, kernel #46)
        ┊
...
(dev, post-1.11.0, untagged) ─ ✦ USB Audio delay + idle heat FIXED — bridge rewritten from a PulseAudio module-loopback to a DIRECT alsaloop --sync=simple bridge (UAC2Gadget → TAS5713, no PA in the audio path): bounded ALSA buffers so the delay can't run away, ~15-20%→~0% CPU, die 93°C→76-79°C, lip-sync confirmed. USB audio now EXCLUSIVE (suspends PA, nq-vol drives the hw mixer). device r65 · APKBUILD depends += alsa-utils (committed 2dccd3a + pushed; OTA published gh-pages d983b3f)   2026-08-09
(dev, post-1.11.0, untagged) ─ ✦ MQTT HEALTH TELEMETRY — new stdlib-Python daemon nexusq-mqtt (0.1.0-r0) publishes retained health JSON + HA MQTT discovery to the home Mosquitto every 30 s → 18 live Home Assistant entities (temp/freq/OPP shares/RSSI/volume/services/problem flags) · device r67 depends += nexusq-mqtt · app 1.12.0 "Device health" panel subscribes to the same feed (apk released as app-v1.12.0) · shipped: commit b49b536 + device-OTA gh-pages cff585f + manifest live · FOLLOW-UP same day: the app PROVISIONS the Q's broker creds (PROTOCOL §13, control r28, app 1.12.1 grey-screen fix → 1.13.0 provisioning; live-proven) · v1.12.0 full image built (all gates PASS, not flashed)   2026-08-10
1.12.0 ─ ✦ OTA everywhere (device · app · full-system) · MQTT health telemetry → HA + app · USB Audio back into PA as a MIXING source (stable-clock snd-aloop hop, device r69/r70 — mixing + unified volume + visualizer restored, live-verified) · iOS app · idle-power made measurable (opp_ms kernel counters, healthd r68)   2026-08-12   ← latest tag
(dev, post-1.12.0, untagged) ─ ✦ IDLE DIET DAY — first clean idle-OPP measurement: 60.5 % @ 350 MHz over a 14 h hands-off MQTT window → same-day per-cgroup attribution (63 % of idle busy CPU = short-lived forks; top consumer = our nq-healthd at ~6.3 % of a core) → four OTA fixes: nq-healthd FORK DIET (device r71: −60 % healthd CPU, idle fork rate 14→3.2/s, JSONL schema unchanged) · nexusqd event-driven PA gate + adaptive 1 Hz idle render cadence (nexusqd r13: 4.4 %→0.165 % of a core, 22→2.9 wakeups/s) · nq_progress measured over a 60 s WINDOW (device r72 — r13's own efficiency had turned zero-tick samples into a CRIT led_frozen on a HEALTHY idle device, twice on the live Q) · LED "stalled" alarm now a device-side VERDICT led_stalled instead of the raw led_stall counter (nexusq-mqtt r2 + new HA "LED ring" binary_sensor — kills the permanent false alarm every idle Q had been raising) ⇒ idle busy 18.2 %→~7.7 % of one core   2026-08-13
(dev, post-1.12.0, untagged) ─ ✦ nexusqd IDLE DIET shipped via OTA (nexusqd r13): event-driven PA sink-input gate (persistent `pactl subscribe` replaces the 1.5 s poll) + adaptive idle render cadence (20 fps → 1 Hz once the frame is bit-identical AND the ring is intent-idle) — 4.4 % → 0.165 % of a core, 22 → 2.9 wakeups/s · ~12 pp of one core of idle background removed across r71+r13   2026-08-13
(dev, post-1.12.0, untagged) ─ ✦ IDLE OPP ROOT-CAUSED + SHIPPED — re-measured after the 08-13 fixes: **70.7 % @ 350 MHz** over 79 h (was 60.5 %) with 1200 MHz/1380 mV all but gone (5.1 %→0.71 %), die 58.4 °C · then a 60 s ftrace + 10 × 12 min governor A/B arms found the mechanism: the idle Q is only **3.8 % busy**, but `conservative` ramps on ANY run ≥16 ms, and the long runs come from **pid 1** (27/min, longest 59 ms, fed by `systemctl` polling) and **nq-healthd** (18/min) — never from the daemons (nexusqd 0.069 %, 0 long runs) · SHIPPED device **r73** / btagent **r5** / mqtt **r3**: `ignore_nice_load=1` + `down_threshold=40` (new nexusq-cpufreq-tune oneshot) + `Nice=19` on the housekeeping units + btagent's `systemctl is-active` → cgroup-dir test ⇒ **72.4 % → 91.0 % verified on the device** (pid 1 collapsed 2 %→0.19 % of a core) with audio ramp responsiveness UNCHANGED · 🔬 known potential test: `sampling_rate=100 ms` + `up_threshold=95` measured **98.7 %** but needs a listening test first · build toolchain PINNED (pmbootstrap 3.11.0 — also un-broke the cold-build blocker)   2026-08-16
(dev, post-1.12.0, untagged) ─ ✦ KERNEL OTA (Phase 2) + A/B ROOTFS — `nexusq-kernel-ota`: a kernel applied without a cable (trial slot p8 → health-gated promote → p9, proven live 6.12.12→r48; attended-only — the stock u-boot has no fallback) · `schedutil` measured and REJECTED (better residency, identical power — it spends 30× longer at 1200 MHz) · then "u-boot ignores the ramdisk" disproven (bad load address — `0x84000000`) → rescue initramfs + **p13/p14 A/B rootfs** (slot marker in p7 misc, health-gated auto-promote, proven both directions) · nq-healthd REWRITTEN IN C (device r77: 3.08→0.55 % of a core, system forks 2.45→0.75/s) · mqtt r4 takes volume from the control bridge · r79 built, boot.img byte-reproducible, trial-booted through its own kernel-OTA path   2026-08-18…21
(dev, post-1.12.0, untagged) ─ ✦ TELEMETRY OUTAGE WORKED BACKWARDS — the missing 5 GHz was the AP auto-selecting DFS ch100 (BCM4330 never reports UNII-2C): router pinned to ch36 → re-associated −52 dBm · healthd rotation leak fixed (r77–r79 wrote into a renamed inode forever, `healthd_fresh:false` in HA; device **r80**, live-verified) · the OTA repo had SILENTLY frozen the fleet at r77 (`OTA_PACKAGES` missing the new `nexusq-rootfs-ab` dep — `apk add --upgrade` exits 0 on an unsatisfiable dep; fixed + republished, rule recorded)   2026-08-23
```
