...
| **B16** | `ramoops: found existing invalid buffer, size 1090782208, start 65604` | ~~Consistent with the known finding that the ramoops region at `0xbf000000` does not survive reboot (DRAM re-init scrubs it)~~ — **that cause was wrong, disproved 2026-09-20.** DRAM survives a reset here: markers written every 2 MB across `0xBF000000`-`0xBFE00000` all came back after a watchdog reset, and so does the ramoops console text itself. What does not survive is the zone **header** (`start`/`size` read back as 0 with intact text behind it), so the buffer looks empty and is discarded. **RESOLVED 2026-09-20 (late):** ramoops works. `systemd-pstore.service` copies every record to `/var/lib/systemd/pstore/` and unlinks it from `/sys/fs/pstore` at boot, so the empty pstorefs that drove years of this analysis proves nothing. A deliberate `sysrq` panic was captured in full (`dmesg-ramoops-0`, 76 kB, with the panic line and backtrace) alongside `console-ramoops-0`. The zone counters reading 0 after a reset is ramoops zapping a buffer it had already handed to pstore. Check crashes with `ls /var/lib/systemd/pstore/`, never `/sys/fs/pstore`. See `docs/2026-09-20-sleep-states-design.md` §4i. Superseded analysis follows: `ramoops.mem_type=1` was built, flashed (kernel `6.18.48-r3`) and tested with a *real* `sysrq` panic — `pstore` stayed EMPTY, so that candidate fix is disproven as well. The header reading was also wrong: the `DBGC` signature **does** survive, and so does the panic text (both found in raw DRAM after the reset). The write path is **healthy**: with correctly-prioritised (`<1>`) messages the console zone's `start`/`size` counters advance normally (0 -> 18790) and the text lands. What does not survive a reset is the header: after a verified reboot the payload is intact but the counters read 0. Since `persistent_ram_zap()` rewrites the signature and zeroes the counters *without* clearing the data, that end state is what ramoops leaves when it distrusts a zone — so the **signature** is the likely casualty, not the counters. See `docs/2026-09-20-sleep-states-design.md` §4i. Original observation: the kernel finds garbage where a previous-boot buffer would be, and logs it at error level. Not new data loss — but it is logged as an error every boot; decide whether to zero the region early or accept + document. | kernel/cmdline |
