...
| `scripts/diag/nq-opp-study2.sh` | device | same harness with arms given on the command line (`label:gov:sampling_rate:up:down:ignore_nice:nice_mode`), incl. renicing the housekeeping units for an `ignore_nice_load` A/B. **This is the instrument to reach for — do not hand-roll a one-off sampler** (it captures `/proc/interrupts` + `/proc/softirqs` + cpuidle, which an ad-hoc sampler forgets and which is exactly where the 2026-08-24 USB-audio cost was hiding). ⚠️ **2026-08-24 fix:** its "playback started, study invalid" guard looked for `librespot`/`shairport-sync`/`roon`/`nexusq-uac2-in` under `system.slice`, but all four are **user** units under `user@10000.service` — that path never exists, so the abort could never fire and a run with music playing would have been reported as idle. It now checks the user manager's tree too. To profile one of those paths **on purpose**, set `ALLOW_UNITS="nexusq-uac2-in.service"` — it drops that unit from the guard and stamps it into `run.log`, so a capture always says which audio path was live |
| `scripts/diag/analyze-opp-snaps.py` | host | study snapshots → per-arm OPP residency, transition matrix, **mean residency per visit**, per-cgroup CPU, IRQ/softirq rates, cpuidle, temperature. Since 2026-08-24 it also prints (a) a **relative dynamic-power index** (C·V²·f weighted by `OPP_MV`, normalised to a locked-350 floor — residency alone misleads, see the schedutil A/B; it reproduces PLAN.md's published "+16 % baseline" and "1200 MHz costs 6.2×" exactly) and (b) the **CPU that is in NO cgroup** (IRQ/softirq/kthreads) as a share of all busy CPU — the blind spot that hid the USB gadget's 1 kHz IRQ cost on 2026-08-24 |
| `perf` (on the device) | device | `perf record -t <tid> -F 499` on a PulseAudio thread is what finally attributed the USB-silence cost (58.86 % of the sink thread inside `libspeexdsp`). The kernel always had `CONFIG_PERF_EVENTS`; only the userspace package was missing, and it is in `device-google-steelhead` `depends=` since **r81** |
...
| `scripts/diag/bench-speex-resampler.py` | device | benchmark libspeexdsp builds on the three resample ratios this box actually runs (48000→48003 USB drift, 1:1, 44100→48000 Spotify), driving each `.so` directly through ctypes. **Two controls, both learned the hard way on 2026-08-24.** (a) *Never* measure a resampler in situ inside PulseAudio — `module-loopback`'s rate controller wanders between exactly 48000 and ~48003 Hz and speex picks a different inner loop from the ratio's **denominator** (1/1 and 147/160 → the NEON-accelerated direct path; 16000/16001 → the interpolate path, no NEON), so two PA samples minutes apart compare different workloads. (b) It **pins the CPU frequency** (governor → `powersave`, 350 MHz) for the run: unpinned, three runs of the identical comparison reported **0.75×, 0.95× and 1.26×** and the first nearly got a good build reverted; pinned, repeats agree to ~1.5 %. On exit it restores the governor **and the `conservative` tunables**, because switching governor RESETS them — that silently reverted a measured `down_threshold` 60 → 20 mid-session |
| `scripts/diag/nq-deep-idle-ladder.sh` | device | one rung of the R2 deep-idle experiment (sleep-states design doc §4m/§4n). It releases the BT UART, which otherwise pins a 170 µs CPU-latency QoS, and clears C2's sysfs USER-disable bit. It then sets the 0048 escape-hatch knobs and samples C2 for N seconds. **It arms the hardware watchdog (30 s) for the duration**, so a hang becomes a warm reset with ramoops intact, not a mains cycle. Everything is restored afterwards |
