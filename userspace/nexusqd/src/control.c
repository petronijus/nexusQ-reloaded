...
int ctl_parse(const char *line, struct ctl_cmd *out) {
    ...
    /* spin R G B [rev_per_s] — optional 5th token sets the rotation speed
     * (revolutions/second, float); omitted/<=0 uses the daemon default. */
    if (!strcmp(tok[0], "spin") && (n == 4 || n == 5)) {
        out->kind = CTL_SPIN;
        if (rgb3(tok[1], tok[2], tok[3], out->rgb) != 0) return -1;
        if (n == 5) {
            char *e; double s = strtod(tok[4], &e);
            if (*e != 0 || !(s > 0.0) || s > 20.0) return -1;
            out->speed = s;
        }
        return 0;
    }
